  ...
  5)
    ...
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
      ...
      sleep 1
                ...
                ;;
  6) open_in_cosmic "$SCRIPT_DIR" "docker-view-logs.sh" "$name" ;;
  [Bb]) return ;;
  [Qq]) clear; exit 0 ;;
  esac
done
...
while true; do
  ...
  if [[ "$choice" == "m" ]]; then open_in_cosmic "$SCRIPT_DIR" "monitor-gpu.sh"; fi
  if [[ "$choice" == "p" ]]; then open_in_cosmic "$SCRIPT_DIR" "docker-list-ports.sh"; fi
